...
#              ps[i].line.replace("\x04}\x04", "}")
    ...
    def match_brach(self, ps, curline, xpos, ypos):
        brachPair = [[')', '('], [']', '['], ['(', ')'], ['[', ']']]
        brach = [')', ']', '(', '[']
        find_brach = False
        if (xpos > 1 and xpos <= len(curline)):        
            chPrev = curline[xpos - 1]
            for index, ch in enumerate(brach):
                if (chPrev == ch):
                    pos_brach = -1
                    if (index < 2):
                        pos_brach = self.rfind_match_brack(xpos - 1, curline, brachPair[index][0], brachPair[index][1])
                    else:
                        pos_brach = self.find_match_brack(xpos - 1, curline, brachPair[index][0], brachPair[index][1])
                    if (pos_brach != -1) :
...
#            print "event: %d"%event
            ...
            if event == idaapi.hxe_curpos:
                ...
                self.match_brach(ps, curline, xpos, ypos)
